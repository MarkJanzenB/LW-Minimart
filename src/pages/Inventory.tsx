...
const Inventory = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    // Status filters - mutually exclusive (only one can be active at a time)
    // Priority: Expired > Low Stock (if both are somehow true, expired takes precedence)
    if (showExpiredOnly) {
              ...
              <div className="flex flex-col lg:flex-row gap-4 items-stretch lg:items-center">
                ...
                <button
                  onClick={() => {
                    ...
                    setShowExpiredOnly(false); // Reset expired filter to enforce mutual exclusivity
                  }}
            ...
            <table className="w-full text-sm">
              ...
              {paginatedData.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right tabular-nums w-24">{formatCurrency(item.price)}</td>
